    ...
    <div class="card mb-6">
      ...
      <form @submit.prevent="handleRequestWithdrawal" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div><label class="label">Amount (₦)</label><input v-model.number="withdrawForm.amount" type="number" min="10000" class="input" required /></div>
          <div><label class="label">Bank Name</label><input v-model="withdrawForm.bankDetails.bankName" type="text" class="input" required /></div>
          <div><label class="label">Account Number</label><input v-model="withdrawForm.bankDetails.accountNumber" type="text" class="input" required /></div>
          <div><label class="label">Account Name</label><input v-model="withdrawForm.bankDetails.accountName" type="text" class="input" required /></div>
        </div>
