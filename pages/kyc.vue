<template>
  <div class="max-w-4xl mx-auto py-8 lg:py-12 animate-fade-in pl-4 md:pl-0 sm:pr-8 pr-4">
    <div class="mb-10">
      ...
      <p class="text-gray-500 font-medium mt-2 text-lg">Submit required documents to verify your merchant account.</p>
    </div>
...
const statusClasses = computed(() => {
  ...
  if (status === 'approved') return {
    ...
    description: 'Your identity has been verified and your merchant account is fully active.'
  }
...
async function fetchKyc() {
  loading.value = true
  try {
    const res = await merchants_api.getKyc() as any
...
async function submitDocument() {
  ...
  try {
    await merchants_api.submitKycDocument(uploadForm)
