import { _ as __nuxt_component_0 } from './nuxt-link-Bc43FneF.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const pageTitle = computed(() => {
      var _a;
      return ((_a = navItems.find((i) => i.path === route.path)) == null ? void 0 : _a.label) || "Merchant";
    });
    return (_ctx, _push, _parent, _attrs) => {
      var _a, _b, _c;
      const _component_NuxtLink = __nuxt_component_0;
      const _component_Icon = __nuxt_component_1;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex h-screen overflow-hidden bg-dark-950" }, _attrs))}><aside class="${ssrRenderClass(["fixed inset-y-0 left-0 z-40 flex flex-col bg-dark-900 border-r border-dark-700 transition-all duration-300", unref(open) ? "w-64" : "w-20"])}"><div class="flex items-center h-16 px-4 border-b border-dark-700"><div class="flex items-center gap-3"><div class="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center font-bold text-white text-sm">M</div>`);
      if (unref(open)) {
        _push(`<span class="text-lg font-bold text-white whitespace-nowrap">Merchant</span>`);
      } else {
        _push(`<!---->`);
      }
      ...
      ssrRenderList(navItems, (item) => {
        _push(ssrRenderComponent(_component_NuxtLink, {
          key: item.path,
          to: item.path,
          class: ["flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all", unref(route).path === item.path ? "bg-emerald-600/10 text-emerald-400 border border-emerald-500/20" : "text-dark-400 hover:text-dark-200 hover:bg-dark-800"]
        }, {
...
//# sourceMappingURL=dashboard-B7u-aqZs.mjs.map
