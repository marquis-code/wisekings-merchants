...
GATEWAY_ENDPOINT.interceptors.request.use(
  (config) => {
    if (typeof document !== 'undefined') {
      const token = document.cookie.split('; ').find(row => row.startsWith('wk_merchant_token='))?.split('=')[1] || '';
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry && !isRefreshRequest) {
      ...
      try {
        ...
        const refreshToken = document.cookie.split('; ').find(row => row.startsWith('wk_merchant_refresh_token='))?.split('=')[1] || '';
        ...
        if (refreshToken) {
          ...
          document.cookie = `wk_merchant_token=${accessToken}${cookieOptions}`;
          document.cookie = `wk_merchant_refresh_token=${newRefreshToken}${cookieOptions}`;
        ...
        console.error("Merchant session expired. Please login again.");
      }
    }
