...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<h1 class="text-2xl font-bold text-white mb-6">Profile</h1><div class="max-w-xl card space-y-4"><div class="text-center mb-4"><div class="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white text-2xl font-bold mx-auto">${ssrInterpolate(unref(user)?.fullName?.[0])}</div></div><div><label class="label">Full Name</label><input${ssrRenderAttr("value", unref(user)?.fullName)} class="input" disabled></div><div><label class="label">Email</label><input${ssrRenderAttr("value", unref(user)?.email)} class="input" disabled></div><div><label class="label">Merchant Code</label><input${ssrRenderAttr("value", unref(user)?.merchantCode)} class="input" disabled></div><div><label class="label">Category</label><input${ssrRenderAttr("value", unref(user)?.category)} class="input capitalize" disabled></div><div><label class="label">Rank</label><input${ssrRenderAttr("value", unref(user)?.rank)} class="input capitalize" disabled></div></div></div>`);
    };
  }
});
...
//# sourceMappingURL=profile-AYmq9LAK.js.map
