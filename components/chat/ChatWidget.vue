<template>
  <div class="fixed bottom-8 right-8 z-[100]">
    ...
    <button 
      ...
      isOpen ? 'bg-gray-900 border border-white/10' : 'bg-emerald-600 shadow-emerald-500/30']"
    >
        ...
        <div class="p-6 bg-gray-900 text-white flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center">
              <Icon name="lucide:headset" class="w-6 h-6 text-emerald-400" />
            </div>
            <div>
              <h3 class="font-black text-sm tracking-tight">WiseKings Support</h3>
              <p class="text-[10px] font-black text-emerald-400 uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/30" ref="messageContainer">
          <div v-if="messages.length === 0" class="flex flex-col items-center justify-center h-full text-center space-y-4">
            <div class="w-16 h-16 bg-white rounded-[1.5rem] flex items-center justify-center shadow-sm">
              <Icon name="lucide:message-square" class="w-8 h-8 text-emerald-500" />
            </div>
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest px-8">Start a conversation with our support team</p>
          </div>
        ...
        <div class="p-4 bg-white border-t border-gray-50">
          <form @submit.prevent="handleSend" class="flex items-center gap-3">
            <input 
              ...
              class="flex-1 bg-gray-50 border-none rounded-2xl py-3.5 px-5 text-xs font-semibold focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
            />
            <button type="submit" :disabled="!newMessage.trim()" class="w-12 h-12 bg-emerald-600 text-white rounded-2xl flex items-center justify-center hover:bg-emerald-700 active:scale-95 transition-all disabled:opacity-50">
...
async function toggleChat() {
  ...
  if (isOpen.value) {
    ...
    const support = conversations.value.find(c => c.type === 'direct') // Simplification
    if (support) {
      await getHistory(support)
    } else {
        // Fallback to chat page if no room to show mini
    }
  }
}
